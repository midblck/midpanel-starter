        ...
        {action.shortcut && action.shortcut.length > 0 && (
          <div className='flex gap-1'>
            {action.shortcut.map((key, index) => (
              <kbd
                key={index}
                className='bg-muted text-muted-foreground pointer-events-none inline-flex h-5 select-none items-center gap-1 rounded border px-1.5 font-mono text-[10px] font-medium opacity-100'
              >
                {key}
              </kbd>
            ))}
