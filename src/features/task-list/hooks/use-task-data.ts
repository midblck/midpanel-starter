...
export function useTaskData({
  ...
  const fetchTasks = useCallback(
    async (
      ...
      try {
        ...
        if (filters.taskTypes) searchParams.append('taskTypes', filters.taskTypes)
        ...
        logDbError('fetch-tasks-hook', err)
        throw err
      }
    },
    [swrMutate]
  ...
  const updateData = useCallback(
    (updater: (currentData: TaskTableData[]) => TaskTableData[]) => {
      swrMutate((current: SWRTaskData | undefined) => {
