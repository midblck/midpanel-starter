    ...
    if (data.type === 'task') {
      ...
      if (active.id !== overId) {
        ...
        if (overColumnPosition !== -1) {
          ...
          const activeIndex = Array.isArray(tasks)
            ? tasks.findIndex(t => t.id === active.id)
            : -1;
          const overIndex = Array.isArray(tasks)
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    if (data.type === 'task') {
      ...
      const isDroppingOnColumn =
        overData?.type === 'column' || columnsId.includes(overId as string);
      ...
      if (isDroppingOnBetweenTaskZone) {
        ...
        if (activeTask.status.id !== columnId) {
          ...
          try {
            ...
            console.error('Failed to insert task at position:', error);
          }
        } else {
        ...
        const targetColumnId =
          overData?.type === 'column' ? overId : overData?.column?.id || overId;
        ...
        if (
          ...
          const columnTasks = tasks.filter(
            t => t.status && t.status.id === activeTask.status.id
          );
