...
const isMaster = (user: UserWithRole): boolean =>
  Boolean(user && user.collection === 'admins' && 'role' in user && user.role === 'master')

// Core access patterns
export const anyone: Access = () => true

export const authenticated: Access = ({ req: { user } }) => isAuthenticated(user)
...
export const createRoleBasedAccess = (allowedRoles: string[]): Access => {
  return ({ req: { user } }) => {
    if (!user || !('role' in user) || typeof user.role !== 'string') return false
